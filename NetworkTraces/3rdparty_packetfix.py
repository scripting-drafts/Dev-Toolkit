...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="PCAPinator - Tool for crazy PCAP analysis")
    parser.add_argument("--in", action="store", dest="infile",
                        help="Input PCAP file or directory", required=True)
    parser.add_argument("--kismetdb", action="store_true",
                        dest="kismetdb", help="Extract PCAP files from Kismet DB")
    ...
    parser.add_argument("--timed", action="store_true", dest="timed",
                        help="Return the processing time of each portion of the code")
    parser.add_argument("--split", action="store_true", dest="split",
                        help="Split the PCAP into pieces based on CPU count")
    parser.add_argument("--split_count", action="store", dest="split_count",
                        help="Set the number of pieces the PCAP will be split into")
    parser.add_argument("--split_output", action="store", dest="split_output",
                        help="Directory location of the newly split files")
    parser.add_argument("--wshark_dir", action="store", dest="wshark_dir",
                        help="Define the location of the wireshark directory")
    parser.add_argument("--handshakes", action="store_true",
                        dest="handshakes", help="Get WPA/WPA2 handshakes")
    parser.add_argument("--wifi_csv", action="store_true",
                        dest="wifi_csv", help="Build CSV files with default tables")
    parser.add_argument("--hashcat", action="store_true",
                        dest="hashcat", help="Output to Hashcat format")
    parser.add_argument("--dnsSimple", action="store_true",
                        dest="dnssimple", help="Create a CSV of dns data only")
    parser.add_argument("--pcapfix", action="store_true", dest="pcapfix",
                        help="Fixes Borked PCAP files, only works in *nix")
    parser.add_argument("--pcapfix_dir", action="store", dest="pcapfd",
                        help="Set the PCAPFix directory where broken files will go")
    parser.add_argument("--min_split", action="store", dest="minsplitsz",
                        help="Min Split Size in bytes, default 200 MB")
    parser.add_argument("--query", action="store", dest="tshark_query",
                        help="A custom query that you want to run on a dataset, use with --fields")
    parser.add_argument("--fields", action="store", dest="tshark_fields",
                        help="The fields list you would like to use with your query, use with --query")
    parser.add_argument("--unique_existing_tsv", action="store_true",
                        dest="existing_tsv", help="Get unique data from an existing TSV file")
    parser.add_argument("--validate_wifi_tsv", action="store_true",
                        dest="validate_tsv", help="Validate and fix a tsv file you are making")
    parser.add_argument("--unique_tsv_field", action="store", dest="unique_tsv_field",
                        help="Default field: ssid. Combine with --unique_existing_tsv. Specify what tshark field you want to base your unique data on (signal, ta, ra, etc.)")
    ...
    if args.tshark_query and args.tshark_fields and args.split:
        ...
        if args.split_output is not None:
            pcapdir = args.split_output
        pcapfiles = getFileSplitsToProcess('.')
        processCustomQuery(pcapfiles, args.tshark_query,
                           args.tshark_fields, pcapdir)
...
def convert2hccapx(**kwargs):
    ...
    if len(kwargs) == 0:
        if os.path.isfile('handshakes.pcap'):
            ...
            if DEBUG:
                print('CMD# "{}" handshakes.pcap {}'.format(
                    CAP2HCCAPX, output_hccapx))
            subprocess.call('"{}" handshakes.pcap {}'.format(
                CAP2HCCAPX, output_hccapx), shell=True)
            if TIMED:
                print(
                    "---- Ran cap2hccapx: {} Seconds ----".format(time.time()-start_time))
    elif 'pcapfile' in kwargs:
        if os.path.isfile(kwargs.get('pcapfile')):
            start_time = time.time()
            if DEBUG:
                print('CMD# "{}" "{}" handshakes.hccapx'.format(
                    CAP2HCCAPX, kwargs.get('pcapfile')))
            subprocess.call('"""{}""" "{}" handshakes.hccapx'.format(
                CAP2HCCAPX, kwargs.get('pcapfile')), shell=True)
            if TIMED:
                print(
                    "---- Ran cap2hccapx: {} Seconds ----".format(time.time()-start_time))
...
def processCSV(pcapfiles, split_output):
    if len(pcapfiles) == 0:
        if DEBUG:
            print("PCAPFILES is length 0 for some reason, something is broken exiting: {}".format(
                pcapfiles))
        return
    if len(pcapfiles) < mp.cpu_count():
        pool = ThreadPool(len(pcapfiles))
    ...
    for f in pcapfiles:
        ...
        results.append(pool.apply_async(
            tsharking, (f, tshark_args, tshark_outargs, tshark_outext, pid, split_output)))
        pid = pid+1
...
def processCustomQuery(pcapfiles, qry, fields, split_output):
    ...
    for f in pcapfiles:
        ...
        tshark_outext = 'tsv'
        results.append(pool.apply_async(
            tsharking, (f, tshark_args, tshark_outargs, tshark_outext, pid)))
        pid = pid+1
...
def processDNSSimple(pcapfiles):
    ...
    if len(pcapfiles) > 1:
        ...
        hasdup = pd.read_csv(tsvfile, sep='\t', lineterminator='\n', names=[
                             'qname', 'rname', 'A_rec', 'AAAA_rec', 'Cname', 'sa', 'ta', 'da', 'ra', 'srv_proto'])
        unique = hasdup.drop_duplicates(
            subset=['qname']).sort_values(by='qname')
        ...
        with open(tsvfile) as f:
            ...
            for line in f:
                ...
                if (quotecnt % 2 != 0) or tabcnt != 16:
                    if DEBUG:
                        print("Broken Line: {} (quotecnt: {}, tabcnt: {})".format(
                            i, quotecnt, tabcnt))
                        print("** FULL LINE: {}".format(line))
                    if tabcnt == 16:
                        #try to fix it
                        nf.write(fixWiFiTSVSSID(line))
                        print("---- Ran fixWiFiTSVSSID on file {}: {} Seconds ----".format(
                            tsvfile, time.time()-start_time))
                else:
                    nf.write(line)
                i = i + 1
    print("---- Finished checkWiFiTSV on File {} in: {} Seconds ----".format(tsvfile,
    ...
    if len(tsvfiles) > 0:
        for tsvfile in tsvfiles:
            if os.stat(tsvfile).st_size < MAXSIZE:
                hasdup = pd.read_csv(tsvfile, sep='\t', lineterminator='\n', names=['time', 'time_epoch', 'sa', 'ta', 'ta_resolved', 'ra', 'da', 'bssid', 'ssid', 'manufacturer', 'device_name', 'model_name', 'model_number', 'uuid_e', 'fc_type_subtype', 'frame_len', 'signal'], dtype={
                                     'device_name': 'object', 'manufacturer': 'object', 'model_name': 'object', 'model_number': 'object', 'uuid_e': 'object', 'fc_type_subtype': 'Int64'})
                if TIMED:
                    print(
                        "---- Read CSV in: {} Seconds ----".format(time.time()-start_time))
                unique = hasdup.drop_duplicates(
                    subset=[UNIQUE_TSV_FIELD]).sort_values(by=UNIQUE_TSV_FIELD)
                unique.to_csv('unique-'+tsvfile, sep='\t', index=False)
            else:
                i = 0
                for chunk in pd.read_csv(tsvfile, sep='\t', lineterminator='\n', names=['time', 'time_epoch', 'sa', 'ta', 'ta_resolved', 'ra', 'da', 'bssid', 'ssid', 'manufacturer', 'device_name', 'model_name', 'model_number', 'uuid_e', 'fc_type_subtype', 'frame_len', 'signal'], chunksize=CHUNKSZ, dtype={'device_name': 'object', 'manufacturer': 'object', 'model_name': 'object', 'model_number': 'object', 'uuid_e': 'object', 'fc_type_subtype': 'Int64'}):
                    if TIMED:
                        print(
                            "---- Read in CSV Chunk {} in: {} Seconds ----".format(i, time.time()-start_time))
                    unique = chunk.drop_duplicates(subset=[UNIQUE_TSV_FIELD])
                    print(tsvfile)
                    unique.to_csv(
                        "chunk_{}-{}.tsv".format(tsvfile[:-4], i), sep='\t', index=False)
                    if TIMED:
                        print(
                            "---- Unique'd CSV Chunk {} in: {} Seconds ----".format(i, time.time()-start_time))
                    i = i + 1
                #Merge chunks back together then unique them
                chunkdir = os.path.abspath(tsvfile)
                tsv_files = [os.path.abspath(fl) for fl in os.listdir(
                    chunkdir) if 'chunk' in fl and fl.endswith('tsv')]
                ...
                if TIMED:
                    print(
                        "---- Finished CSV Chunking in: {} Seconds ----".format(time.time()-start_time))

        if TIMED:
            print(
                "---- Ran Unique-ing process in: {} Seconds ----".format(time.time()-start_time))
    else:
...
def mergeCSV(outdir):
    ...
    tsv_files = [os.path.abspath(os.path.join(OUT_PATH, fl)) for fl in os.listdir(
        OUT_PATH) if 'split' in fl and fl.endswith('tsv')]
...
def mergePcaps():
    ...
    start_time = time.time()
    subprocess.call(
        '"{}" -F pcap -w handshakes.pcap hs*.pcap'.format(MERGECAP), shell=True)
    if TIMED:
...
def processpcapfix(pcapfiles):
    ...
    for f in pcapfiles:
        ...
        if DEBUG:
            ...
            print('"{}" -o "{}-fixed{}" "{}"'.format(PCAPFIX,
                                                     os.path.join(fpath, filename), file_extension, f))
        subprocess.call('"{}" -o "{}-fixed{}" "{}"'.format(PCAPFIX,
                                                           os.path.join(fpath, filename), file_extension, f), shell=True)
...
def splitpcap(inpcap, outdir, splitcnt):
    ...
    try:
        packet_count = int(subprocess.check_output(
            '"{}" -K -c -M -T -m "{}"'.format(CAPINFOS, inpcap), shell=True, encoding='utf8').split(',')[-1])
    ...
    if filesz <= MINSPLIT:
        if DEBUG:
            ...
            print('CMD# cp {} {}split_{}'.format(
                inpcap, outdir, filename_w_ext))
        shutil.copyfile(inpcap, "{}split_{}".format(outdir, filename_w_ext))
        if TIMED:
            print(
                "---- Ran shutil.copyfile: {} Seconds ----".format(time.time()-start_time))
    else:
        if DEBUG:
            print('CMD# "{}" -F pcap -c {} "{}" {}split_{}.pcap'.format(EDITCAP,
                                                                        chunk_size, inpcap, outdir, filename))
        subprocess.call('"{}" -F pcap -c {} "{}" {}split_{}.pcap'.format(EDITCAP,
                                                                         chunk_size, inpcap, outdir, filename), shell=True)
        if TIMED:
...
def tsharking(inpcap, params, output, outext, procid, outdir='.'):
    ...
    if output == '' and outdir == '.':
        ...
        p = subprocess.Popen('"{}" -r "{}" {}'.format(TSHARK, inpcap, params),
        ...
        outfullpath = ''
        # if outdir != '.':
        outfullpath = os.path.join(os.path.abspath(
            outdir), "{}.{}".format(output, outext))
        ...
        if DEBUG:
            print('{} -r "{}" {} >> "{}"'.format(TSHARK,
                                                 inpcap, params, outfullpath))
        p = subprocess.Popen('"{}" -r "{}" {} >> "{}"'.format(TSHARK, inpcap, params,
                                                              outfullpath), stdout=subprocess.PIPE, stderr=subprocess.PIPE, shell=True)

    out, err = p.communicate()
    if TIMED:
        print("---- ProcID: {} Ran tshark: {} Seconds FILE: {} ----".format(procid,
                                                                            time.time()-start_time, inpcap))
    if DEBUG:
        if out:
...
def kismetLog2Pcap(kismetdb, pid):
    ...
    for row in c.execute(sql):
        if logf == None:
            if DEBUG or VERBOSE:
                print("Id: {} Assuming dlt {} for all packets".format(
                    pid, row[2]))
