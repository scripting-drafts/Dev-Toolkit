...
favourites = ['Matsumoto Kappa Bridge', 'Shizuoka Mount Fuji', 'Sendai Airport',
              'Kushiro Harbour']
...
def add_to_map(df, row):
    ...
    iframe = folium.IFrame(html(img_url,url,url_frontend), width=220, height=180)  # Original 420 236
...
for row in range(0, len(df.index)):
    t = Thread(target=add_to_map, args=(df, row))
    threads.append(t)
    t.start()
